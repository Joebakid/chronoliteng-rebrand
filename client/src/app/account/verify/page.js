...
export default function VerifyAccountPage() {
  ...
  useEffect(() => {
    ...
    async function doVerify() {
      ...
      if (!token) {
        if (isMounted) {
          setStatus({ loading: false, error: "Verification token missing.", message: "" });
        }
        return;
      }
      ...
      try {
        ...
        if (isMounted) {
          setStatus({ loading: false, message: payload?.message || "Email verified. You can now sign in.", error: "" });
        }
      } catch (error) {
        if (isMounted) {
          setStatus({ loading: false, error: error.message || "Unable to verify email right now.", message: "" });
        }
      }
    }
