...
/**
 * HELPER: resolveOrderImage
 * Ensures we check 'selectedVariantImage' (saved during checkout) 
 * before falling back to the default product image.
 */
            ...
            {loading ? (
              ...
              paginated.map((purchase) => (
                <div 
                  ...
                  className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 transition-all hover:border-[var(--accent)] hover:shadow-md cursor-pointer group active:scale-[0.98]"
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[0.65rem] text-[var(--muted)] font-medium">
                      {formatDate(purchase.createdAt)} • <span className="italic opacity-60">View details</span>
                    </p>
                  </div>
                  ...
                  <div className="flex -space-x-2.5">
                    {purchase.items.slice(0, 3).map((item, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="h-8 w-8 rounded-full border-2 border-[var(--surface)] bg-white overflow-hidden shadow-sm"
                        >
                          <img 
                            src={itemImg} 
                            alt={item.name} 
