...
export default function CartView() {
  ...
  async function handleCheckout(paystackRef = null) {
    setCheckoutError("");
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      ...
      <aside className="h-fit rounded-[1.75rem] border border-[var(--border)] bg-[var(--surface-strong)] p-6 shadow-[var(--shadow)]">
        ...
        {checkoutError && (
          <div className="mt-4 rounded-2xl border border-[rgba(161,69,59,0.2)] bg-[rgba(161,69,59,0.08)] px-4 py-3 text-sm text-[var(--danger)]">{checkoutError}</div>
        )}
        ...
        <div className="mt-5 space-y-3">
          <button type="button" onClick={() => handleCheckout()} className="w-full rounded-full bg-[var(--foreground)] px-6 py-3 text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[var(--surface-strong)]">
            Proceed to checkout
          </button>
          <button type="button" onClick={handlePaystack} disabled={paystackLoading} className="w-full rounded-full border border-[var(--border)] px-6 py-3 text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[var(--foreground)] disabled:opacity-50">
            {paystackLoading ? "Loading Paystack…" : "Pay with Paystack"}
          </button>
          {paystackError && <p className="mt-1 text-xs text-[var(--danger)]">{paystackError}</p>}
          <button type="button" onClick={clearCart} className="w-full rounded-full border border-[var(--border)] px-6 py-3 text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[var(--foreground)]">
            Clear cart
          </button>
        </div>
      </aside>
