    ...
    <header className="sticky top-0 z-50 pt-4 px-4">
      <nav className="site-frame nav-shell rounded-[2rem] bg-[var(--nav)] px-4 py-2 backdrop-blur-xl border border-[var(--border)] shadow-[var(--shadow)] transition-all duration-300">
        ...
        <div className="flex items-center justify-between w-full gap-2">
          ...
          >
            <span className="text-[1.3rem] font-bold tracking-tighter text-[var(--foreground)]">
              C.
            </span>
            <span className="font-display text-[0.95rem] font-bold text-[var(--foreground)] hidden xs:block">
              Chronolite
            </span>
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              href="/cart"
              className="flex items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface-strong)] px-3 py-1.5 sm:px-4 sm:py-2 text-[0.7rem] font-bold uppercase tracking-wider transition hover:bg-[var(--background-strong)]"
            >
              ...
              {user ? (
                <>
                  <span className="text-[0.68rem] font-semibold text-[var(--muted)] px-2 max-w-[120px] truncate">
                    ...
                    {user.isAdmin && (
                      <span className="ml-1 text-[0.55rem] font-bold uppercase tracking-widest text-amber-500">
                        Admin
                      </span>
                    )}
                  </span>
                  ...
                  <Link
                    href={accountHref}
                    className="text-[0.7rem] font-bold uppercase tracking-wider px-3 py-2 hover:opacity-70"
                  >
                    {accountLabel}
                  </Link>
            ...
            >
              {menuOpen ? (
                ...
                <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
                  <path d="M1 1H15M1 6H15M1 11H15" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                </svg>
              )}
            </button>
          </div>
        ...
        {menuOpen && (
          <div className="mt-3 pt-3 border-t border-[var(--border)] flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
            {user ? (
              <>
                <div className="rounded-xl bg-[var(--surface)] px-4 py-2.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link
                    href={accountHref}
                    className="rounded-xl bg-[var(--background-strong)] p-4 text-center text-[0.7rem] font-bold uppercase"
                  >
                    {accountLabel}
                  </Link>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Link
                  href="/account/create-account"
                  className="rounded-xl bg-[var(--inverse-bg)] text-[var(--inverse-fg)] p-4 text-center text-[0.7rem] font-bold uppercase"
                >
                  Sign Up
                </Link>
              </div>
